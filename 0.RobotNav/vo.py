...
def run_vo(shared_pose=None, stop_flag=None):

    # Load intrinsic matrix (ganti path sesuai foldermu)
    ...
    try:
        while True:
            ...
            # Share pose ke thread lain
            if shared_pose is not None:
                shared_pose['pose'] = cur_pose

            if stop_flag is not None and stop_flag.get('stop', False):
                print("[VO] Stop flag detected, exiting.")
                break

            # Optional: print pose for debug
            # print(f"[VO] Pose: {cur_pose}")

            end = time.perf_counter()
            total_time = end - start
            fps = 1 / total_time if total_time > 0 else 0

            time.sleep(0.01)  # biar CPU tidak 100%
