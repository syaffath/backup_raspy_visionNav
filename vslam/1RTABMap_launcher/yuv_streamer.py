...
class YUVStreamer:
    def __init__(self, pipe_path="live.yuv", output_dir="dataset", width=640, height=480, fps_limit=20):
    ...
    def run(self):
        ...
        with open(self.pipe_path, "rb") as pipe:
            while True:
                ...
                # FPS info setiap 100 frame
                if self.frame_id % 100 == 0:
                    total_elapsed = time.time() - start_time
                    avg_fps = self.frame_id / total_elapsed
                    print(f"📊 {self.frame_id} frames in {total_elapsed:.2f}s → avg {avg_fps:.2f} FPS")
