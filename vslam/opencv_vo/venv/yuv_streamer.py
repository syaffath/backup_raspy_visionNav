...
class YUVStreamer:
    def __init__(self, pipe_path, width, height):
        ...
        # Pose (tanpa rotasi global)
        self.position = np.zeros((3, 1))  # akumulasi t langsung
        ...
        self.ax.set_title("Live VO Trajectory (frame-to-frame t)")
        self.ax.grid(True)
        self.ax.axis("equal")
    ...
    def yuv420_to_gray(self, yuv):
        y_only = yuv[:self.width * self.height]  # hanya ambil Y (tanpa UV)
        gray = np.frombuffer(y_only, dtype=np.uint8).reshape((self.height, self.width))
        return gray


    def run(self):
        with open(self.pipe_path, 'rb') as pipe, open("trajectory_log_OF.csv", "w", newline="") as logfile:
            ...
            while True:
                raw = pipe.read(self.frame_size)
                if len(raw) < self.frame_size:
                    continue

                gray = self.yuv420_to_gray(raw)
                ...
                if len(good_prev) >= 8:
                    ...
                    if avg_flow < 0.5:
                        ...
                        if E is not None:
                            ...
                            if 0.01 < t_norm < 1.0:
                                self.position += t  # tanpa rotasi, langsung tambah vektor t
                                x, y, z = self.position.flatten()
